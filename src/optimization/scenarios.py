...
def compare_scenarios(
    ...
    if len(df) > 0:
        ...
        df["response_vs_base"] = (df["expected_response"] - base_response) / base_response * 100
...
def create_channel_shift_scenarios(
    ...
    for shift_pct in shift_amounts:
        ...
        new_allocation[shift_channel] = max(
            0, current_allocation.get(shift_channel, 0) + shift_amount
        )
        ...
        if other_total > 0:
            for ch in other_channels:
                ...
                new_allocation[ch] = max(
                    0, current_allocation.get(ch, 0) - shift_amount * proportion
                )
