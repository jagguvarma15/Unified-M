...
class OptimizationResult:
    ...
    def save(self, path: Path | str) -> None:
        ...
        if self.channel_metrics is not None:
            self.channel_metrics.to_csv(path.with_suffix(".csv"), index=False)
...
class BudgetOptimizer:
    ...
    def optimize(
        ...
        for channel in self.channels:
            ...
            current_spend = (
                self._current_allocation.get(channel, 0) if self._current_allocation else 0
            )
            ...
            channel_data.append(
                {
                    ...
                    "spend_change_pct": (optimal_spend - current_spend)
                    / (current_spend + 1e-8)
                    * 100,
