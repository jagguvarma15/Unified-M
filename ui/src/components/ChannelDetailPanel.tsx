...
function fmt(v: number | undefined, prefix = "", suffix = ""): string {
  ...
  if (Math.abs(v) >= 1_000_000)
    return `${prefix}${(v / 1_000_000).toFixed(1)}M${suffix}`;
  if (Math.abs(v) >= 1_000)
    return `${prefix}${(v / 1_000).toFixed(1)}k${suffix}`;
          ...
          <div class="p-6 space-y-6">
            <div class="grid grid-cols-2 gap-4">
              ...
              <StatBox
                label="ROAS"
                value={
                  props.channel!.roas != null
                    ? `${props.channel!.roas.toFixed(2)}x`
                    : "—"
                }
              ...
              <StatBox
                label="Lift"
                value={
                  props.channel!.lift != null
                    ? props.channel!.lift.toFixed(4)
                    : "—"
                }
            ...
            <Show
              when={
                props.channel!.current != null && props.channel!.optimal != null
              }
            >
              <div class="rounded-lg border border-slate-200 p-4">
                ...
                <div class="space-y-3">
                  <BudgetBar
                    label="Current"
                    value={props.channel!.current!}
                    max={Math.max(
                      props.channel!.current ?? 0,
                      props.channel!.optimal ?? 0,
                    )}
                    color="bg-slate-400"
                  />
                  <BudgetBar
                    label="Optimal"
                    value={props.channel!.optimal!}
                    max={Math.max(
                      props.channel!.current ?? 0,
                      props.channel!.optimal ?? 0,
                    )}
                ...
                <Show
                  when={
                    props.channel!.current != null &&
                    props.channel!.optimal != null
                  }
                >
                  {(() => {
                    const diff =
                      props.channel!.optimal! - props.channel!.current!;
                    const pct =
                      props.channel!.current! > 0
                        ? (diff / props.channel!.current!) * 100
                        : 0;
                    return (
                      <p
                        class={`mt-3 text-xs font-medium ${diff > 0 ? "text-emerald-600" : diff < 0 ? "text-red-600" : "text-slate-500"}`}
                      >
                        {diff >= 0 ? "+" : ""}
                        {fmt(diff, "$")} ({pct >= 0 ? "+" : ""}
                        {pct.toFixed(1)}%) to reach optimal
                      </p>
                    );
                  })()}
                </Show>
