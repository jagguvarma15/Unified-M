...
const BLOCK_TEMPLATES: {
...
}[] = [
  {
    type: "text",
    label: "Text Block",
    defaultTitle: "Section Header",
    defaultContent: "Add your narrative here...",
  },
  {
    type: "chart",
    label: "Chart",
    defaultTitle: "Chart Widget",
    defaultContent: "contributions",
  },
  {
    type: "metric",
    label: "KPI Card",
    defaultTitle: "Key Metric",
    defaultContent: "r_squared",
  },
  {
    type: "table",
    label: "Data Table",
    defaultTitle: "Data Table",
    defaultContent: "channel_summary",
  },
];
  ...
  const [blocks, setBlocks] = createSignal<ReportBlock[]>([
    {
      ...
      content:
        "This report summarizes the key findings from the latest MMM run.",
    },
    {
      id: "2",
      type: "metric",
  ...
  const exportAsText = () => {
    ...
    for (const b of blocks()) {
      ...
      if (b.type === "text") lines.push(b.content);
        ...
        lines.push(
          `[Chart: ${CHART_OPTIONS.find((c) => c.value === b.content)?.label ?? b.content}]`,
        );
      else if (b.type === "metric")
        lines.push(
          `[Metric: ${METRIC_OPTIONS.find((m) => m.value === b.content)?.label ?? b.content}]`,
        );
            ...
            {(block, i) => (
              ...
              >
                <div class="flex items-start gap-3">
                  <div
                    class="mt-1 cursor-grab text-slate-300 hover:text-slate-500 transition-colors"
                    draggable
                  >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      <span
                        class={`inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold ${blockTypeColor(block.type)}`}
                      >
                    ...
                    {block.type === "chart" && (
                      <div class="h-32 rounded-lg bg-gradient-to-br from-indigo-50 to-slate-50 flex items-center justify-center">
                        <p class="text-xs text-indigo-400 font-medium">
                          {CHART_OPTIONS.find((c) => c.value === block.content)
                            ?.label ?? "Chart"}
                        </p>
                      </div>
                    )}
                    {block.type === "metric" && (
                      <div class="inline-flex items-center gap-2 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2">
                        <span class="text-xs text-slate-500">
                          {METRIC_OPTIONS.find((m) => m.value === block.content)
                            ?.label ?? "Metric"}
                        </span>
        ...
        <div class="w-72 shrink-0">
          <div class="sticky top-20 rounded-xl border border-slate-200 bg-white shadow-sm p-4 space-y-4">
            ...
            <Show
              ...
              fallback={
                <p class="text-xs text-slate-400 py-8 text-center">
