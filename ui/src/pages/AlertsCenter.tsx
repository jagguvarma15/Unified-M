...
import { Bell, Plus, Trash2, CheckCircle2, AlertTriangle, X, AlertCircle } from "../lib/icons";
...
const CHANNELS = [
  "All Channels", "google_ads", "meta_ads", "tiktok_ads", "linkedin_ads",
  "pinterest_ads", "snapchat_ads", "twitter_ads", "apple_search_ads",
];

const DEMO_RULES: AlertRule[] = [
  { id: "r1", name: "Low ROAS Alert", channel: "All Channels", metric: "roas", operator: "<", threshold: 1.5, enabled: true, lastTriggered: "2026-04-03T14:22:00Z" },
  { id: "r2", name: "High CPA Warning", channel: "meta_ads", metric: "cpa", operator: ">", threshold: 50, enabled: true, lastTriggered: null },
  { id: "r3", name: "Saturation Alert", channel: "google_ads", metric: "saturation", operator: ">", threshold: 0.85, enabled: false, lastTriggered: "2026-03-28T09:15:00Z" },
];

const DEMO_EVENTS: AlertEvent[] = [
  { id: "e1", ruleId: "r1", ruleName: "Low ROAS Alert", channel: "tiktok_ads", metric: "roas", value: 1.2, threshold: 1.5, severity: "critical", timestamp: "2026-04-03T14:22:00Z", acknowledged: false },
  { id: "e2", ruleId: "r1", ruleName: "Low ROAS Alert", channel: "snapchat_ads", metric: "roas", value: 1.35, threshold: 1.5, severity: "warning", timestamp: "2026-04-02T10:45:00Z", acknowledged: true },
  { id: "e3", ruleId: "r3", ruleName: "Saturation Alert", channel: "google_ads", metric: "saturation", value: 0.92, threshold: 0.85, severity: "warning", timestamp: "2026-03-28T09:15:00Z", acknowledged: true },
];
...
export default function AlertsCenter() {
  ...
  const [newOperator, setNewOperator] = createSignal<AlertRule["operator"]>("<");
  ...
  const severityColor = (severity: string) =>
    severity === "critical" ? "text-red-600 bg-red-50 border-red-200" : "text-amber-600 bg-amber-50 border-amber-200";
  ...
  return (
    <div>
      ...
      <div class="grid grid-cols-3 gap-3 mb-6">
        ...
        <MetricCard label="Enabled Rules" value={enabledRules()} color="emerald" />
      </div>
      ...
      <div class="flex gap-1 mb-4">
        <button
          ...
          class={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab() === "events" ? "bg-indigo-600 text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"
          }`}
        >
          Alert Events
          <Show when={activeAlerts() > 0}>
        ...
        <button
          ...
          class={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab() === "rules" ? "bg-indigo-600 text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"
          }`}
        >
        ...
        <Show
          ...
          fallback={
            <div class="rounded-xl border border-dashed border-slate-300 bg-white py-16 text-center">
              ...
              <p class="mt-1 text-xs text-slate-400">Events appear when configured thresholds are exceeded</p>
            </div>
          }
        >
          <div class="space-y-2">
                ...
                return (
                  ...
                  >
                    <div class="flex items-start gap-3">
                      <SevIcon size={18} class={event.severity === "critical" ? "text-red-500 mt-0.5" : "text-amber-500 mt-0.5"} />
                      <div class="flex-1">
                        <div class="flex items-center gap-2">
                          <span class="font-semibold text-sm">{event.ruleName}</span>
                        ...
                        <p class="text-sm mt-0.5">
                          ...
                          {" "}{event.metric} = <span class="font-mono tabular-nums font-semibold">{event.value}</span>
              ...
              <div class="flex items-center gap-4 rounded-lg border border-slate-200 bg-white p-4">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-slate-500">
                    {rule.channel} · {METRICS.find((m) => m.value === rule.metric)?.label ?? rule.metric} {rule.operator} {rule.threshold}
                  </p>
                </div>
      ...
      <Show when={showAdd()}>
        <div class="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <div class="relative w-full max-w-md rounded-xl bg-white shadow-xl p-6 space-y-4">
            <div class="flex items-center justify-between">
              <h3 class="text-base font-semibold text-slate-900">New Alert Rule</h3>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={newOperator()}
                  onChange={(e) => setNewOperator(e.currentTarget.value as AlertRule["operator"])}
                  class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                >
              ...
              <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Threshold</label>
                <input
                  type="number"
                  step="0.01"
                  value={newThreshold()}
