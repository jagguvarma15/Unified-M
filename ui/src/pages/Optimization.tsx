...
export default function Optimization() {
  ...
  const [view, setView] = createSignal<"grouped" | "change" | "allocation">("grouped");

  const channels = () => (data() ? Object.keys(data()!.optimal_allocation) : []);
      ...
      <Show when={data()} fallback={<EmptyState />}>
        <div>
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-slate-900">Budget Optimization</h1>
              <p class="text-sm text-slate-500 mt-1">Optimal budget allocation to maximize expected response</p>
            </div>
            <a
              href="/scenarios"
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
            ...
            <MetricCard
              ...
              value={data()!.expected_response.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              icon={TrendingUp}
              color="emerald"
            />
            <MetricCard
                    ...
                    <For each={[...optimalPie()].sort((a, b) => b.value - a.value)}>
                      {(ch, i) => (
                        <div class="flex items-center gap-3">
                          ...
                          <span class="text-sm text-slate-700 flex-1">{ch.name}</span>
          ...
          <div class="bg-white rounded-xl p-6 shadow-sm border border-slate-200/60 mt-6">
            <h2 class="text-sm font-semibold text-slate-700 mb-4">Channel Recommendations</h2>
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-slate-200">
                    <th class="text-left py-3 px-4 font-semibold text-slate-600">Channel</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Current</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Current %</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Optimal</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Optimal %</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Change</th>
                    <th class="text-right py-3 px-4 font-semibold text-slate-600">Change %</th>
                  ...
                  <For each={channelDetails()}>
                    {(ch, i) => (
                      <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        ...
                        <td class="text-right py-3 px-4 tabular-nums">${ch.cur.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                        <td class="text-right py-3 px-4 tabular-nums text-slate-500">{ch.curShare.toFixed(1)}%</td>
                        <td class="text-right py-3 px-4 tabular-nums">${ch.opt.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                        <td class="text-right py-3 px-4 tabular-nums text-slate-500">{ch.optShare.toFixed(1)}%</td>
                        <td class={`text-right py-3 px-4 tabular-nums font-medium ${ch.diff > 0 ? "text-emerald-600" : ch.diff < 0 ? "text-red-500" : "text-slate-500"}`}>
                          {ch.diff >= 0 ? "+" : ""}${ch.diff.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </td>
                        <td class={`text-right py-3 px-4 tabular-nums font-medium ${ch.pct > 0 ? "text-emerald-600" : ch.pct < 0 ? "text-red-500" : "text-slate-500"}`}>
                          {ch.pct >= 0 ? "+" : ""}{ch.pct.toFixed(1)}%
                        </td>
                        <td class="py-3 px-4">
