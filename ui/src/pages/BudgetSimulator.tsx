...
export default function BudgetSimulator() {
  ...
  const [curvesData, setCurvesData] = createSignal<ResponseCurvesData | null>(null);
  ...
  const totalAllocated = () => channels().reduce((s, ch) => s + channelBudget(ch), 0);
  ...
  return (
    ...
    >
      <Show when={optData()} fallback={<EmptyState title="No optimization data" hideQuickStart />}>
        <div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            ...
            <MetricCard
              label="Est. Response"
              value={estimatedResponse().toLocaleString(undefined, { maximumFractionDigits: 0 })}
              changePct={upliftPct()}
              changeLabel="vs current"
          ...
          <div class="bg-white rounded-xl border border-slate-200/60 shadow-sm p-6 mb-6">
            <div class="flex items-center justify-between mb-3">
              <h2 class="text-sm font-semibold text-slate-700">Total Budget Multiplier</h2>
              <span class="text-lg font-bold tabular-nums text-indigo-600">{(totalBudgetMult() * 100).toFixed(0)}%</span>
            </div>
            <input
              type="range"
              min={30}
              ...
              onInput={(e) => setTotalBudgetMult(Number(e.currentTarget.value) / 100)}
          ...
          {/* Per-channel sliders */}
          <div class="bg-white rounded-xl border border-slate-200/60 shadow-sm p-6 mb-6">
            <h2 class="text-sm font-semibold text-slate-700 mb-4">Channel-Level Adjustments</h2>
            <div class="space-y-4">
              <For each={channels()}>
                {(ch, i) => {
                  ...
                  return (
                    <div class="flex items-center gap-4">
                      <div class="w-28 flex items-center gap-2">
                        ...
                        <span class="text-sm font-medium text-slate-700 truncate">{ch.replace(/_spend$/, "")}</span>
                      </div>
                      <div class="flex-1">
                        <input
                          type="range"
                          ...
                          step={5}
                          value={mult() * 100}
                          onInput={(e) => updateChannelMult(ch, Number(e.currentTarget.value) / 100)}
                          class="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                        />
                      </div>
                      <div class="w-20 text-right">
                        <span class="text-sm font-mono tabular-nums">{formatCurrency(adjusted(), true)}</span>
                      </div>
                      <div class="w-20 text-right">
                        <span class={`text-xs font-medium tabular-nums ${diff() > 0 ? "text-emerald-600" : diff() < 0 ? "text-red-500" : "text-slate-400"}`}>
            ...
            >
              <ReactChart>
                {() => h(ResponsiveContainer, { width: "100%", height: 300 },
                  h(BarChart, { data: channelCompare(), layout: "vertical", margin: { left: 70 } },
                    ...
                    h(Bar, { dataKey: "simulated", name: "Simulated", fill: "#6366f1", radius: [0, 3, 3, 0], barSize: 10 })
                  )
                )}
              </ReactChart>
            </ChartCard>
              ...
              <ReactChart>
                {() => h(ResponsiveContainer, { width: "100%", height: 300 },
                  h(LineChart, { data: efficiencyData() },
                    h(CartesianGrid, { strokeDasharray: "3 3", stroke: CHART_GRID }),
                    h(XAxis, { dataKey: "budget", tick: { fontSize: 11 }, tickFormatter: (v: number) => formatSpendTick(v) }),
                    h(YAxis, { tick: { fontSize: 11 }, tickFormatter: (v: number) => formatCompactNumber(v) }),
                    h(Tooltip, {
                      contentStyle: { background: CHART_TOOLTIP_BG, border: "none", borderRadius: 8, fontSize: 12, color: "#e2e8f0" },
                      formatter: (v: number) => [v.toLocaleString(undefined, { maximumFractionDigits: 0 }), "Response"],
                      labelFormatter: (v: string) => `Budget: $${Number(v).toLocaleString()}`,
                    }),
                    h(ReferenceLine, { x: totalAllocated(), stroke: "#6366f1", strokeWidth: 2, strokeDasharray: "4 4", label: { value: "Simulated", fontSize: 10, fill: "#6366f1" } }),
                    h(ReferenceLine, { x: baseBudget(), stroke: "#94a3b8", strokeDasharray: "4 4", label: { value: "Current", fontSize: 10, fill: "#94a3b8" } }),
                    h(Line, { type: "monotone", dataKey: "response", stroke: "#10b981", strokeWidth: 2, dot: false })
                  )
                )}
              </ReactChart>
            </ChartCard>
