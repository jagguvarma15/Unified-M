...
export default function Dashboard() {
  ...
  const contributions = () => (contributionsQ.data ?? null) as ContributionsData | null;
  const reconciliation = () => (reconciliationQ.data ?? null) as ReconciliationData | null;
  const optimization = () => (optimizationQ.data ?? null) as OptimizationData | null;
  const runs = () => (runsQ.data ?? null) as RunsData | null;
  const waterfall = () => (waterfallQ.data ?? null) as WaterfallData | null;
  const diagnostics = () => (diagnosticsQ.data ?? null) as DiagnosticsData | null;
              ...
              <PageHeader
                ...
                detail={
                  <span class="inline-flex items-center gap-1.5">
                    ...
                    Run: <code class="font-mono text-slate-500">{run().run_id.slice(0, 12)}…</code>
                  </span>
                }
                  ...
                  rightSlot={
                    diagnostics()!.residual_stats ? (
                      <div class="flex items-center gap-3 text-xs text-slate-500">
                        <Show when={diagnostics()!.residual_stats!.mean != null}>
                          <span>Mean: <span class="font-mono font-medium text-slate-700">{diagnostics()!.residual_stats!.mean!.toFixed(1)}</span></span>
                        </Show>
                        <Show when={diagnostics()!.residual_stats!.std != null}>
                          <span>Std: <span class="font-mono font-medium text-slate-700">{diagnostics()!.residual_stats!.std!.toFixed(1)}</span></span>
                        </Show>
                      </div>
                    ) : undefined
                  }
